...
<style>
body {
    background: linear-gradient(145deg, #0a0f24, #1b2a49);
    color: #f2f2f2;
...
def display_image(img, rotate=False):
    ...
    st.markdown(f"""
        ...
        style="border-radius:20px; box-shadow:0 0 40px #0047b3; width:100%; transform:{transform};">
    """, unsafe_allow_html=True)
...
def run_classifier(img):
    ...
    st.markdown(f"""
        ...
        style="background: conic-gradient(#4da6ff {conf*100}%, #0a0f24 0%);">
        {conf*100:.1f}%
        </div>
    """, unsafe_allow_html=True)
